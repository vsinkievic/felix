        ...
        then do:
            ...
            if not available ttUsed
                then do:
                    ...
                    ttUsed.isUsed = false.
                    //display ttUsed.info format "x(40)". 
                end.
        end.
    end.
...
function detailLevel returns integer (compileUnitNext as character):
    ...
    find first ufiles no-lock where ufiles.compileUnit matches ("*" + compileUnitNext + "*") and
        ...
        (ufiles.type = "COMPILE" or ufiles.type = "INCLUDE") no-lock no-error.
        if available ufiles then do: 
        ...
        vCompileUnit = ufiles.info. //cia tikrinamo dalyko info laukas
            ...
            for each bfiles no-lock where 
                ...
                bFiles.compileUnit = ufiles.compileUnit 
                  and bFiles.system = vSystem and
                  ((index(bfiles.info, ".p") <> 0)  
                  or (index(bfiles.info, ".cls") <> 0) 
                  or (index(bfiles.info, ".i") <> 0))):      
                          
    /*            create unused.                          */
    /*            unused.compileUnit = bFiles.compileUnit.*/
    /*            unused.fileName = bFiles.fileName.      */
    /*            unused.type = bFiles.type.              */
    /*            unused.system = bFiles.system.          */
                //display bFiles.info with size 50 by 10.
...
/*                end.                                                     */
     ...
     for each bUsed where bUsed.isUsed = false no-lock no-error:
                    create unused no-lock no-error.
